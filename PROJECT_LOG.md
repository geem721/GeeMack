...
live domain over to the React app today, with a new "phone piece" (Twilio?) planned for
tomorrow. Cutover (Phase 6) retires `public/index.html` — but Video Call (Phase 5) wasn't
built yet, and the legacy app's video calling is real, currently-used functionality.
Asked directly rather than assume: build Video Call first and cut over once it's
verified, or cut over now and accept video calling disappears from the live site until
Phase 5 ships. **User chose: build and verify Video Call first.** Cutover is gated on
that, not on a deadline.

**Built this session:**
- `web/src/rooms.js` — extracted the fixed 5-room list (`general/support/travel/
  business/casual`) out of `GroupChat.jsx` into its own module, same "one shared source"
  rationale as `languages.js`. Video Call needs the identical list (see below) and a
  second hand-typed copy would just reintroduce the drift this migration exists to fix.
- `web/src/tabs/VideoCall.jsx` + `.css` — feature-parity port of legacy video calling:
  LiveKit video/audio (`livekit-client` from npm, `^2.22.0`, replacing legacy's unpinned
  `cdn.jsdelivr.net/npm/livekit-client` `<script>` tag — versioned and bundled properly
  instead of silently picking up upstream changes), Deepgram live captions relayed
  through the existing `/ws/transcribe` WebSocket (no `server.js` changes needed — that
  endpoint already existed and is untouched), per-listener translated caption overlays,
  gated by `AuthGate` (reused as-is from Phase 4).
  - **Real design gap this session had to resolve, not spelled out in
    `MIGRATION_PLAN.md`**: legacy tied video calls to whichever room Group Chat happened
    to be showing, because the call button lived inside the Group Chat panel. Now that
    Video Call is its own top-level tab (2026-08-16 decision), it needs its own room
    concept. Resolved by giving it the same fixed room picker as Group Chat
    (`rooms.js`) — consistent mental model, and the Firebase paths
    (`chats/{room}/captions`) still line up with what Group Chat already established.
    Own invite-link flow added (`?tab=videocall&room=X`), separate from but consistent
    with Group Chat's.
  - **Deliberately deferred, flagged not silently dropped: call recording.**
    `MIGRATION_PLAN.md` calls this out as genuinely new functionality ("record the video
    call itself, not just mic audio") — it's never existed in any live version of this
    app, so deferring it doesn't take anything away from a real user. Doing it properly
    needs a canvas compositor for the video tiles plus a WebAudio-mixed multi-track audio
    stream into one `MediaRecorder` — real, separate scope. Not built this session; not
    blocking cutover, since cutover only needs to preserve what's currently live.
  - Video/audio track handling stays imperative (direct DOM manipulation mirroring
    legacy's `attachTrack`/`showCaption`, via a `gridRef`) rather than modeled as
    declarative React state — same reasoning `CameraOCR.jsx` already established for its
    raw `<video>` element: LiveKit hands back tracks with their own attach()/detach()
    lifecycle that doesn't map cleanly onto props/state.
- **Bundle-size fix, caught before it shipped**: adding `livekit-client` on top of
  Phase 4's Firebase pushed the main JS bundle from 468kB to 970kB, tripping Vite's
  500kB warning — every visitor landing on Translate (still the most-used tab, zero
  calling/auth dependencies) would have downloaded both SDKs before touching either
  feature. Fixed by lazy-loading `GroupChat` and `VideoCall` via `React.lazy()` +
  `Suspense` in `App.jsx`, same "heavy feature-specific dependency shouldn't tax every
  visitor" reasoning `Documents.jsx` already applies to `pdfjs-dist`/`mammoth`/`xlsx`/
  `jszip` via dynamic `import()`. Main bundle back down to 223kB; `VideoCall`'s own chunk
  (livekit-client, ~500kB) only loads when someone actually opens that tab.
- **CSS dedup**: moved the room-picker/account-header/pill/lang-bar/invite-modal styles
  Video Call needed to share with Group Chat out of `GroupChat.css` into `shared.css`
  (kept the `gc-` class-name prefix as-is to avoid touching Phase 4's already-verified-
  live markup) — same instinct that put `.btn`/`.lang-bar` primitives in `shared.css`
  back in Phase 2, not duplicated per-tab.
- `App.jsx` — extended invite-link URL parsing to `?tab=videocall&room=X`; an untagged
  `?room=X` (every link generated by Phase 4 before Video Call existed) still defaults to
  Group Chat, so no previously-shared invite link breaks. Subtitle bumped to "Phase 5:
  Video Call tab."

**Verified in this sandbox** (no live LiveKit/Firebase/Deepgram reachable here — same
limitation as every backend-dependent phase): `npm install && npm run build` succeeds
clean, bundle-size warning resolved. `oxlint` clean (same pre-existing `Toast.jsx`
fast-refresh warning as every prior phase). Headless Playwright smoke test against the
production build: Video Call tab renders the sign-in gate correctly; Group Chat and
Translate both still render correctly (regression check on the CSS move and lazy-
loading); `?tab=videocall&room=support` activates the Video Call tab; a legacy-style
`?room=business` link (no `tab` param) still activates Group Chat. No console/page
errors outside the expected sandbox network restrictions.

**Not yet verified live** — needs a real test on Apollo1: join a call from two
accounts/devices, confirm video/audio both ways, confirm live captions appear
translated on each side, confirm the room picker and invite link work the same way
Group Chat's did.

**Next: deploy this patch, get Phase 5 confirmed live the same way Phase 4 was (real
accounts, real devices), then proceed to Phase 6 (cutover) — point talk-bridge.org's
main URL at the React build, retire `public/index.html` (kept in git history). After
cutover: research the "phone piece" (Twilio?) the user mentioned — not in
`MIGRATION_PLAN.md` today, needs real scoping (pricing, how it sits alongside
LiveKit/Firebase, whether it extends Phase 5 or is its own phase) before proposing
